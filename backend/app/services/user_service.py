...
class UserService:
    """Business operations for user registration and account status."""

    def __init__(self):
        """Initialize the user data-access dependency."""
        self.repository = UserRepository()

    def create_user(self, session: Session, user_in: UserCreate) -> User:
        """Create a user unless their phone number is already registered."""
    ...
    def deactivate_user(self, session: Session, user_id: UUID) -> User:
        """Mark an existing user as inactive without deleting them."""
        user = self.repository.get(session, user_id)
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
